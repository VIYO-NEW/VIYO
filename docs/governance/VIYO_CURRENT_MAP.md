...
| **Decision Cataloger** | Claude Code Desktop (formalizes at Phase 3.2 CLAUDE.md v2 ratification) | Commits ratified decisions to Notion canonical state. Refuses commits without explicit PO ratification surface. | When ratified decisions need cataloging to Notion DBs. (Transitional period: Architect performs canonical writes only with per-write PO ratification per ARCHITECT_OPERATING_RULES §3.4 edge case.) |
