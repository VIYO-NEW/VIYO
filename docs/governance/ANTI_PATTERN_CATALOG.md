...
### 1.4 Post-2026-05-18 Observations (Section F-supplemental)

Two observations surfaced during 2026-05-18 B-0.02 directive arc + session-start handoff failure. Supplements the original 15 Section F observations from VIYO_FOUNDATIONAL_SEED.md without renumbering the canonical 15.

**Observation 16 — Variant 5g 6-cycle meta-recurrence (B-0.02 directive v0.1 → v0.6, 2026-05-18).** Architect authored §0 standing-rule self-acknowledgment explicitly logging variant 5g (memory-citation without grep-verify). Within the SAME §0 block, Architect cited "EF-28 verbatim" as canonical authority for the agentic-rule scope correction. Adversary v4 grep at HEAD `f980a76` returned ZERO matches for EF-28 in Foundation Locks / R-specs / PRD V8.1 / VVOW Architecture / governance. Only 1 match in `MANUS_VIYO_Feature_Inventory_By_Phase.md` (feature description, NOT canonical authority). Variant 5g recurred INSIDE the §0 block authored to refute it. Cycle progression: v0.1 had 5 variant 5g instances; v0.2/v0.3/v0.4 had 2 each (Adversary caught + Architect fixed by introducing new ones); v0.5 added 1 (Lock 19/Rule 7/Rule 11 conflation); v0.6 had 4 (§0 verbatim citation defects — Lock 19 Statement truncated, R29 §10 citation from wrong location, PRD §3.4 truncated mid-sentence, Lock 19/38 relationship contaminated). Fix framework: Family 4 (External Validation) + new Category 5 (Tool-Level Pre-Commit Enforcement) — pre-commit citation verifier queued as Phase 1B Curator-tool deliverable. Pattern conclusion: human-discipline-only insufficient — pattern persisted across 6 consecutive Adversary cycles despite explicit standing-rule logging within each cycle. Family classification: Family 5 (memory-citation) + Family 7 (rule accumulation meta-pattern) + universal Rule E violation.

**Observation 17 — Inventory feature descriptions cited as canonical authority (2026-05-18).** Architect cited "EF-28" (feature description in `MANUS_VIYO_Feature_Inventory_By_Phase.md`) as canonical authority for agentic-rule scope extension during B-0.02 directive v0.4 authoring. Inventory documents are feature catalogs (descriptions of what features WILL be built or HAVE been built); they are NOT canonical authority for architectural principles. Canonical authority hierarchy for principles (per Authority Hierarchy 12-tier statement in FOUNDATION_LOCK.md): Foundation Locks → R-specs → PRD V8.1 → Notion Decision Log. Inventory documents NEVER canonical authority for principles. Inventory citations valid only for feature scope, feature status, feature dependencies, feature priority. Standing rule (per ARCHITECT_OPERATING_RULES Rule 3.18 ratified 2026-05-18): "Inventory feature descriptions are NEVER canonical authority for architectural principles — never cite as architectural source." Family classification: Family 5 (memory-citation applied to authority hierarchy) + new variant under universal Rule E violation. Fix framework: Family 2 (Schema Enforcement — clarify Inventory document role in inheritance) + Family 4 (External Validation via pre-commit citation verifier — flags Inventory citations in directive authority claims).

**Observation 18 — Notion DB ID memory-citation without verify (2026-05-18; migrated from NOTION_CATALOGING_RULES.md §7 to canonical catalog 2026-05-19).** Architect session memory carried Notion DB IDs from prior session compaction that did not match canonical structure. 6 of 7 IDs were wrong (only Conflict Resolutions matched). Mechanism: Notion's structure has database PAGE URLs (what humans see) AND data source URLs (what tools query) — Architect memory conflated these. Variant 5c (memory-citation universal) applied to Notion canonical structure. Closes via Rule 3.19 (project_knowledge_search FIRST applied to Notion structure verification) + NOTION_CATALOGING_RULES.md as canonical reference. Architect must fetch the actual database via `notion-fetch` before any write — never rely on memory of IDs.

**Observation 19 — PO Inbox staleness gap (2026-05-18 discovery; migrated from NOTION_CATALOGING_RULES.md §7 to canonical catalog 2026-05-19).** PO Inbox had 9 substantive design proposals undrained for 6+ days (oldest from 2026-05-12). No agent had standing rule to monitor. Failure pattern: surface exists; routing field exists; no protocol enforces monitoring. Closes via NOTION_CATALOGING_RULES.md §3 PO Inbox Monitoring Protocol locked as standing rule across all agent session starts (chained into Rule 3.20 §6 First Action step 2.5 per follow-on commit 2026-05-19).

**Observation 20 — Meta-recursive variant 5c at Notion-structure layer inside authoring file (2026-05-19).** NOTION_CATALOGING_RULES.md §1.3 + §2.7 cited Standing Rules page ID `3559a84a467981d5c9f3e2b7f3a5d9f4`. CCD Cataloger session 2026-05-19 attempted to update that page during drain Section 5 execution; notion-fetch returned 400 validation_error. Workspace search revealed actual page ID `35c9a84a-4679-81c7-a1fc-db7556f572d4` (title: "⚖️ Standing Rules"). The Architect-authored canonical authority document on Notion cataloging discipline contained an instance of exactly the pattern its §7 Observation 18 was added to prevent (variant 5c memory-citation at Notion-structure layer). Mechanism: Architect session memory carried wrong page ID into NOTION_CATALOGING_RULES.md authoring 2026-05-18. Authoring document did not match its own canonical-structure verification mandate (Rule 3.19 project_knowledge_search FIRST). CCD Cataloger caught the drift during execution (used verified correct ID for the actual write; surfaced content drift; did NOT silently edit the file per Rule 3.4). Pattern classification: META-RECURSIVE variant 5c — authoring rules-against-X while exhibiting X inside the authoring. Fix framework: Family 4 (External Validation) — CCD Cataloger's pre-write verification caught the drift; closes prospectively via pre-commit citation verifier (Phase 1B Curator-tool per Category 5) which will grep-verify every cited Notion page ID against actual page existence before commit. Also strengthens Rule 3.19 application: when authoring a file that codifies canonical-structure rules, EVERY cited ID in the file requires explicit notion-fetch verification before commit. Family classification: Family 5 (memory-citation) + Family 7 (rule accumulation meta-pattern) + universal Rule E violation.
...
### Category 5 — Tool-Level Pre-Commit Enforcement (added 2026-05-18)

**Definition:** Automated tooling at the commit boundary detects citation defects, missing verbatim text, and mechanism-citation drift BEFORE the commit lands. Empirically required after 6-cycle variant 5g recurrence (B-0.02 directive v0.1 → v0.6 with Adversary v1 → v6) proved human-discipline-only insufficient.

**Structural change:** Beyond external validation (Category 1) which runs at the PO-surfacing boundary, Category 5 runs at the GIT COMMIT boundary. Hooks substrate (native git-hooks per commit `9a98f23`) operationalizes the commit-msg + pre-push portions; the pre-commit citation verifier (Phase 1B Curator-tool deliverable) closes the conversational-layer 5g (citation in directive bodies that reach commit boundary).

**Mechanism: WHO/WHAT enforces it**

| Enforcement layer | Who | What it checks | When it runs |
|---|---|---|---|
| Native commit-msg hook (existing per commit `9a98f23`) | `scripts/git-hooks/commit-msg` | SHA citations verified via `git cat-file -e`; file paths with separators verified against working tree; conventional commit format + ratification citation enforced | At every governance commit |
| Native pre-push hook (existing per commit `9a98f23`) | `scripts/git-hooks/pre-push` | Drift-pattern sweep with file allowlist — blocks push of files containing `FOUNDATION_LOCKS_v2`, `NIR_OPERATING_RULES_v2`, `ARBITRATION_FOUNDATION_LOCK_v2` outside allowlist | At every push |
| Native post-commit hook (existing per commit `9a98f23`) | `scripts/git-hooks/post-commit` | Appends commit metadata to `~/.viyo/session-log.md` for durable session-log substrate | After every commit |
| **NEW: Pre-commit citation verifier** (Phase 1B Curator-tool, Manus dispatch pending) | TBD — Bash/TypeScript Curator-tool with git pre-commit integration | Tool extracts every quoted block in staged files labeled with `source:line` or `source §N`; `grep`-verifies the exact quoted string is present at the named location; compares quoted text to actual text (detects truncation, paraphrase, wrong location); blocks commit if any quoted block contains text not present at cited location | At every governance commit + every directive commit |
| **NEW: Pre-dispatch verbatim-text check** (Phase 1B Curator-tool) | Pre-commit citation verifier (same tool) | When directive surfaces R-spec edit / Foundation Lock edit / governance file edit, verifier confirms Architect-authored verbatim text supplied INLINE; blocks ratification flow if directive assigns Architect-tier work to Manus without verbatim per variant 5k | Pre-dispatch + at every directive commit |
| **NEW: Mechanism-citation empirical-test check** (Phase 1B Curator-tool) | Pre-commit citation verifier (same tool) | When directive cites a tool/API/system MECHANISM (behavior, contract, sequencing) as the basis for a fix, verifier requires accompanying empirical test evidence (synthetic case run output); blocks commit if mechanism citation lacks empirical test per variant 5h | At every infrastructure-touching commit (hooks, build configs, deploy scripts, MCP integrations) |

**Empirical justification:** Required per 6-cycle variant 5g recurrence (B-0.02 directive v0.1 → v0.6 with Adversary v1 → v6). Variant 5g manifested in EVERY cycle despite explicit standing-rule logging at Architect-level within each cycle. Pattern conclusion: human-discipline-only insufficient.

**Addresses variants:** 5a (commit-boundary closure), 5b (file-level sweep), 5c (commit-boundary + conversational-layer via verifier), 5d (capability claims in commits), 5f (drift-pattern sweep), 5g (commit-boundary + Layer 2 closure via verifier), 5h (empirical-test check), 5k (verbatim-text pre-dispatch check). **Does NOT address:** 5e (governance-register pollution — operational discipline, not tool-level), Family 6 (real-time challenge signal interruption — harness-level).

**Status:** Native hooks LIVE since commit `9a98f23` (2026-05-18). Pre-commit citation verifier queued as Phase 1B Curator-tool deliverable per Pending Work Queue item #4. Manus dispatch directive pending Architect authoring per Item 1 Phase C downstream.

**Reference:** Adversary v6 review of B-0.02 v0.6 (2026-05-18) explicitly recommended this category: tool-level pre-commit verifier is now empirically reinforced — 6 cycles of human-discipline-only have not closed variant 5g.

---

### §3.5 — Anti-Pattern Variants (5-prefix nomenclature, 8 variants + 1 added 2026-05-18)

Anti-pattern variants surfaced during 2026-05-15 + 2026-05-18 session work. Originally 4 variants (a/b/c/d) per 2026-05-15 session; renamed to 5-prefix nomenclature per A9 §5 Catalog Migration Action 2026-05-18; expanded with 5e-5h from A9 §5 verbatim + 5k added at Item 1 commit 2026-05-18.

**5a. Stale-citation-propagation chain (3-layer)** — variant of Family 1. Example: D63 K1=COPY case (Architect cited stale → Curator paste-back propagated → PO ratified the stale chain). Three governance layers all skipped grep-verifying the cited substrate against the Decisions DB. A3 Interpretation 2 unwittingly superseded D63. Resolution: Path α realign 2026-05-15 reaffirming D63; A3 I2 reversed to I1. Closes via Rule E grep-verify mandate + native commit-msg hook (commit `9a98f23`) for commit-boundary SHA + file-path verification.

**5b. Arbiter adjacent-string blindness** — Arbiter flags stale citation in one location but doesn't grep same string across rest of touched file. X2.18/X2.20 example (timeline "12-17" fixed at line 67, missed at adjacent line 339 same file). Closes via Rule D strengthened mandate (stale-string sweep across all touched files for any flagged stale citation) + native pre-push hook (commit `9a98f23`) for file-level drift-pattern sweep.

**5c. Memory-citation (universal)** — agent cites from memory / inference / paraphrase / skim instead of grep-verifying canonical source. Forbidden across all agents (Architect / Curator / PO-side Claude / Arbiter / Adversary / Manus / Cataloger). Closes via Rule E (universal precondition resolving all stale-citation families at source) + isolated Adversary cold-read for conversational-layer closure per A9 §9 Addition 5.

**5d. Capability-citation without verify** — subspecies of 5c. Agent asserts "I don't have access to X tool" without verifying via tool_search with multiple keyword variants. Closes via Rule E applied to own tool access.

**5e. OD-register category drift** — variant surfaced 2026-05-15; PO ratified wording 2026-05-18 (kept as "OD-register category drift" per PO Q5 answer in A9 §12). Example: OD-006 spend-cap-as-OD — operational hygiene item (monthly spend cap monitoring) registered as Open PO Decision, conflating operational tasks with project-level architectural decisions. Degrades OD register signal-to-noise. Closes via OD registration criteria (only items requiring PO product/scope/architecture decision qualify); operational items route to separate operational checklist. Family classification: governance-register pollution.

**5f. Version-suffix-in-filename drift** — variant surfaced 2026-05-15, closed at commit `b1b5496` (variant 5f sweep) 2026-05-18. Example: Governance files temporarily named with `_v2.md` suffix during authoring (`FOUNDATION_LOCKS_v2.md`, `NIR_OPERATING_RULES_v2.md`). The `_v2` suffix propagated through cross-references in adjacent files even after ratification, creating drift between filename and citation. Closes via stable-filename discipline (Lock 2026-05-15) + variant 5f sweep commit + native pre-push hook drift-pattern sweep (commit `9a98f23`) which blocks future push of drift patterns outside allowlist. Family 1 variant.

**5g. Memory-citation of fix-target without grep-verify (TWO-LAYER)** — variant surfaced 2026-05-15 + 2026-05-18; 7+ instances logged through 2026-05-18. Definition: agent cites a SHA, decision number, file path, Notion page ID, capability, or other canonical fact AS the fix-target of a proposed/committed action, WITHOUT grep-verifying the cited fact exists in current canonical state. Two-layer reality: Layer 1 commit-boundary (citation in commit message or committed file — CLOSED structurally by `scripts/git-hooks/commit-msg` per commit `9a98f23`); Layer 2 conversational-layer (citation in chat reasoning that doesn't reach commit boundary — NOT closed by hooks; closure via isolated Adversary cold-read per A9 §9 Addition 5). **6-cycle recurrence in B-0.02 directive v0.1 → v0.6 (2026-05-18)** confirms human-discipline-only insufficient — tool-level enforcement empirically required (see new Category 5 in §4). Family 1 variant + universal Rule E violation. Subspecies of 5c distinguished by citation being FIX-TARGET of action.

**5h. Memory-citation of mechanisms without empirical test** — variant surfaced 2026-05-18 via pre-commit hook failure case study. Architect Claude designed a `pre-commit` git hook assuming `.git/COMMIT_EDITMSG` is populated before the hook fires — true for editor-flow commits, false for `git commit -m "msg"` workflow. CCD Curator's verification tests caught the bug before propagation. Path α correction moved SHA-citation + file-path verify logic to `commit-msg` hook (reliable across `-m`, `-F`, editor flows). Distinct from 5c (memory-citation of FACTS) because cited item is a MECHANISM (behavior, contract, sequencing). Closes via empirical test before propagating mechanism citation + "verify before dispatch" discipline on infrastructure-touching directives. Subspecies of 5c applied to mechanisms.

**5k. Architect-tier work assigned to Manus without verbatim text** (added 2026-05-18 at Item 1 commit) — Manifestation B-0.02 directive v0.2 → v0.3 Adversary v2 Finding #2: Path B (auth.ts service-role bypass documented via R22 v2 §9.3 sites list) initially assigned R-spec authoring of new §9.3 row to Manus without Architect-supplied verbatim text. Per ARCHITECT_OPERATING_RULES §3 Rule 3.4 (R-spec ratification → Manus commit pattern): Architect authors R-spec text; Manus commits Architect-authored text. Variant 5k = directive assigns Architect-tier work to Manus without Architect-supplied verbatim text inline. Closes via Family 3 (Agent Topology Separation) — Curator-tool implementation note: when reviewing directives surfaced for ratification, verify every R-spec edit / Foundation Lock edit / governance file edit has Architect-supplied verbatim text INLINE before commit. Architect-tier work requires Architect-supplied verbatim per Rule 3.4 R-spec ratification → Manus commit pattern.

**External enforcement:** Rule E (locked in ARCHITECT_OPERATING_RULES §3) requires grep-verify against canonical source before propagating any citation. Rule D strengthened mandate requires Arbiter stale-string sweep across all touched files. Rule 3.10 (SESSION_STATE repo-canonical) closes the local-only-state subset. Together, Rules A/B/C/D/E + Rule 3.7-3.10 chain + native git-hooks substrate (commit `9a98f23`) + isolated Adversary cold-read (A9 §9 Addition 5) + new Category 5 pre-commit citation verifier (queued Phase 1B) forms the universal anti-stale-citation discipline locked across all relay boundaries (PO ↔ PO-side Claude ↔ Curator ↔ Arbiter ↔ Adversary ↔ Manus ↔ Cataloger).

For full reasoning chains + concrete session examples see `SESSION_LEDGER_2026-05-15.md` + B-0.02 directive v0.1 → v0.6 archived in commit history (Item 1 commit 2026-05-18).
