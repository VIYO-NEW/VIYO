...
- **Lock 22 RATIFIED** (Authority Hierarchy Time-Horizon Split) + **Lock 39 candidate placeholder** (Agentic-vs-Architectural Boundary, deferred to Substrate Map work post-B-0.02 v1.0) + **Lock 40 RATIFIED** (Source-Never-Shown Invariant — Licensed-Inspiration Pattern Seeding gate; Notion `3679a84a4679818cbbf7c73eea41907b`; activates when Pattern Seeding code first touches repo; paired with OD-024).
...
- **D-2 LANDED 2026-05-20** (ledger anchor AF `4ae061c` via PR #38 merge): receiver-side pre-dispatch citation verifier `scripts/verifier/verify-dispatch.sh` + shared `lib/citation-patterns.sh` + NEW AOR Rule 3.23 mandate that Curator runs verify-dispatch.sh at every dispatch intake. 6 Curator-call fixes applied vs original dispatch: (1) drop UUID v4 validation in shared lib — variant 5c instance #9 at dispatch-authoring layer (same bug pasted forward despite D-1 fix); (2) port Lock candidate/placeholder exemption from D-1; (3) add context-cue gating to Notion ID detection; (4) iterate ALL hex matches per SHA context-cue line; (5) fixture 07 PASS-with-INFO semantics; (6) drop markdown anchor self-link in Rule 3.23. Plus inline CTX_WINDOW bash-substring bug fix. 8 dispatch fixtures + runner; all PASS. Closes 4 of 6 historical variant 5g instances at dispatch boundary.
- **Pre-push case-insensitivity fix LANDED 2026-05-20** (ledger anchor AG `f316917` via PR #39 merge): D-1 Lock-verify candidate/placeholder exemption was case-sensitive `grep -vE` — "Lock 39 Candidate" (capitalized) not exempted. Fixed to `grep -viE`. Surfaced by Curator pre-flight on the NOTION_CATALOGING_RULES.md re-sync. Discrete ratified item per Option A (separate small PR, keeps re-sync commit scope clean).
- **Git hooks RE-INSTALLED 2026-05-20** — CRITICAL substrate finding: the installed `.git/hooks/pre-push` was a stale copy predating D-1 (drift-pattern-only, no citation-verify). `install-hooks.sh` creates file COPIES not symlinks on Windows, and pre-push was never re-installed after D-1 landed — so D-1's "Tool-Level Pre-Commit Enforcement" was DORMANT from PR #37 merge until 2026-05-20. `bash scripts/install-hooks.sh` re-run 2026-05-20; installed pre-push now current (citation-verify + grep -viE active). Side-queue: `install-hooks.sh` symlink reliability fix queued (copies silently go stale on every script change).
- **NOTION_CATALOGING_RULES.md re-sync 2026-05-20** (this commit; ledger anchor AH reserved per A9 §6 Convention): re-verified against live Notion after the 2026-05-19 Manus reorg (VIYO_NOTION_CLEANUP_001) + 2026-05-20 hygiene audit. §1 workspace two-page top-level structure; §2.1 Decision ID ceiling D54→D96/next D97 + `Domain` multi-select 8-option property; §2.1-V five named Decisions DB views (verified-live config); §2.7 Hygiene Rules H-1–H-6 index; §1.3 child-page IDs corrected (prior entries were invented — variant 5c in the repo doc); §5/§7 marked historical. Verification pass per Rule E surfaced + closed: Manus's View Setup Guide page fix initially left 2 residual artifacts, resolved post-verification — page re-verified live clean 2026-05-20 (§2.1-V note).
...
- **AOR Rule 3.24 RATIFIED 2026-05-27** (this commit; ledger anchor AK reserved per A9 §6 Convention): UI/UX Pro Max skill local clone `D:\Dropbox\Claude\ui-ux-pro-max-skill` adopted for per-domain query usage inline in dispatches. `--design-system` whole-baseline mode REJECTED — two empirical tests (2026-05-27) mis-categorized VIYO Image Studio (Invoice/Billing, Marketing Agency) and produced landing-page patterns instead of app surfaces. Skill verified local-only / no network calls / CSV-backed. Architect-side Claude Code skill installed at `~/.claude/skills/` (out-of-repo). Notion Session Log 2026-05-24 page receives 2026-05-27 entry in same session (Rule 3.22 trigger). Bundled in this commit: AI/AJ ledger catch-up for pre-existing unledger'd governance commits `377e77e` (Phase 6 cleanup 2026-05-22) and `a7c01c8` (§9.5 pre-launch seeding) — drift closed.
...
*2026-05-27 architect-side design tooling adoption COMPLETE. AOR Rule 3.24 ratified + SESSION_STATE bundled refresh per Rule 3.21 (this commit = AK anchor, per A9 §6 Convention caught up next commit; AI/AJ catch-up for prior Phase 6 cleanup + §9.5 pre-launch seeding drift bundled in same commit). UI/UX Pro Max skill local-clone adopted for per-domain queries inline in dispatches; auto-baseline mode rejected after empirical mis-categorization. Session Log 2026-05-24 page updated with 2026-05-27 entry per Rule 3.22. Architect-side Claude Code skill installed at `~/.claude/skills/`. Pending queue unchanged (item 1 = viyo-zcbr-architect skill v1.0.7 still next; item 2 = B-0.02 v1.0 re-author).*
